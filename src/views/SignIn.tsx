...
export default function SignIn() {
  ...
  const handleSendCaptcha = async () => {
    try {
      ...
      const { data: captcha }: HttpResponse = await sendCaptcha({ phone: signInfo.phone })
          ...
          <div className="mt-3 w-full flex justify-between items-center">
            <div className="bg-gray-200 w-8/12 h-10 rounded px-3">
              <input
                ...
                onChange={(e) => setSignInfo({ ...signInfo, captcha: e.target.value })}
              />
            </div>
            <span className={`w-3/12 h-10 flex items-center justify-center px-8 py-3 border border-transparent text-sm font-medium rounded-md text-white md:py-4 md:text-lg md:px-10 ${countdownStyles}`} onClick={handleSendCaptcha}>
              {isSend ? `${countdown}s` : 'Send'}
            </span>
          </div>
        </div>
        <span className={`mt-6 flex items-center justify-center px-8 py-3 border border-transparent text-sm font-medium rounded-3xl text-white md:py-2 md:text-lg md:px-6} ${signInStyles}`} onClick={handleSignIn}>
          Sign In
        </span>
      </div>
    </div>
